...
`pacman -U`. The checked-in recipes use an explicit checksum placeholder because `0.0.0` is
not a public release. The release workflow substitutes the tag archive or
bundle SHA-256, runs `makepkg --printsrcinfo`, builds both packages in clean
Arch containers, and publishes the resulting recipes with the release assets.
