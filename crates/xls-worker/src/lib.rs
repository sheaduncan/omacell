//! Calamine-backed BIFF conversion for the resource-limited XLS worker.
//!
//! This crate is not linked into the Omacell host. [`parse_xls_bytes`] must be
//! called only by the companion process after it installs its resource limits.
#![forbid(unsafe_code)]
#![deny(missing_docs)]
...
use calamine::{CellErrorType, Data, Reader, SheetType, SheetVisible, Xls};
...
/// Parse legacy BIFF bytes inside the resource-limited worker process.
///
/// The Calamine parser can abort on crafted input, so production callers must
/// not invoke this function in the host process.
pub fn parse_xls_bytes(bytes: &[u8]) -> Result<Workbook, CoreError> {
