...
pub fn export(wb: &Workbook) -> Result<Vec<u8>, CoreError> {
    ...
    for col in used.min_col..=used.max_col {
        keys.push(cell_text(wb, sheet, used.min_row, col));
    }
...
fn objects(value: &Value) -> Result<Vec<Map<String, Value>>, CoreError> {
    ...
    for item in items {
        match item {
            Value::Object(map) => rows.push(flatten(map, "")),
            other => {
                let mut map = Map::new();
        ...
        if rows.len() as u32 > MAX_ROWS {
            return Err(error::xlsx_limit("JSON array exceeds the row grid"));
        }
    }
    Ok(rows)
}
...
fn flatten(map: &Map<String, Value>, prefix: &str) -> Map<String, Value> {
    ...
    for (k, v) in map {
        ...
        match v {
            Value::Object(inner) => out.extend(flatten(inner, &key)),
            Value::Array(items) => {
                for (i, item) in items.iter().enumerate() {
                    let ik = format!("{key}[{i}]");
                    match item {
                        Value::Object(inner) => out.extend(flatten(inner, &ik)),
                        other => {
                            out.insert(ik, other.clone());
                        }
                    }
                }
            }
...
fn table_to_workbook(rows: &[Map<String, Value>]) -> Result<Workbook, CoreError> {
    ...
    for (r, row) in rows.iter().enumerate() {
        let row_i = (r as u32).saturating_add(1);
        for (c, key) in keys.iter().enumerate() {
...
fn cell_json(wb: &Workbook, sheet: omacell_core::addr::SheetId, row: u32, col: u16) -> Value {
    match wb.get(sheet, row, col).ok().flatten() {
        Some(slot) => match slot.value {
            omacell_core::value::Value::Number(n) => serde_json::Number::from_f64(n)
                .map(Value::Number)
                .unwrap_or(Value::Null),
            omacell_core::value::Value::Bool(b) => Value::Bool(b),
            omacell_core::value::Value::Text(id) => {
                Value::String(wb.intern().strings.get(id).unwrap_or("").to_string())
            }
            _ => Value::Null,
        },
        None => Value::Null,
