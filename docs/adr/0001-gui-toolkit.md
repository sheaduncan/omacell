...
WP-S2 did not fail IME or accessibility *outright*. The 1M-row scroll, theme
swap, fractional-scale, AccessKit, and fontconfig checks that could be run on
this Omarchy 4 / Hyprland box all land inside the §12.1 budgets. CJK IME and
Orca speech were not executable here (no IM engine, no Orca package); they
are human re-tests at gate G4, not a toolkit swap.
...
| Startup to first frame | GUI < 300 ms | **143 ms** (repeat 197 ms) | Empty spike window, not product chrome. |
...
Stay on egui unless one of these triggers fires. A Qt Quick front-end is a
...
2. **Accessibility outright fail.** Orca is running and cannot read the
   focused cell's address and value from the AccessKit/AT-SPI tree, and the
   tree itself is missing the node (not just Orca's speech). WP-S2 already
   produced the node; G4 must confirm Orca.
...
Missing packages on a spike box (no Orca, no pinyin) are **not** triggers.
