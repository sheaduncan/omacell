...
Register a named AI prompt, optional JSON schema, and tools in the retained AI runtime.

## `omacell.ai.fn`

`omacell.ai.fn(name, spec)`

Register a namespaced asynchronous AI worksheet function in the retained AI runtime.

## `omacell.on_ai_request`

`omacell.on_ai_request(fn)`

Transform a privacy-filtered AI request before provider dispatch; host APIs are unavailable in the hook.

## `omacell.on_ai_response`

`omacell.on_ai_response(fn)`

Transform a validated provider response before caching; host APIs are unavailable in the hook.
