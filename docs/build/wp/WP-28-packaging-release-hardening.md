...
- `PKGBUILD` (source) and `-bin` variant; `.desktop` with MIME associations (`.xlsx`, `.xlsm`, `.xls`, `.csv`, `.tsv`, `.ods`, `.omc`), MIME XML, `hicolor` icons plus symbolic variant, completions/man/skill/default files installed under `/usr/share/omacell`; post-install message pointing to `omacell setup omarchy`; `makepkg` build in an Arch container in CI.
...
- Performance gate workflow: the §12.1 table as benches with committed baselines and a 10 % budget; `just perf-baseline` documented. GUI cold-start and scroll measurements must include wgpu render/presentation work on the fixed integrated-GPU host, not only egui layout/tessellation. Recheck WP-16's software-kittest baselines (about 334 ms release cold start and 9.3 ms steady-state scroll after review) against the product targets.
...
- GUI completion pass: resolve WP-16's remaining release gaps (multi-window decision/implementation, painted split panes, per-cell family/fallback fidelity and shaping-cache profiling, clickable status segments), and exercise each path in the keyboard/mouse/a11y walkthroughs. WP-17 clipboard/fill/atomic drag and WP-18 conditional-format overlay consumption are already covered by pre-WP-28 worker, keyboard, mouse, pixel, and terminal-rendering regressions.
- Print release closure: replace WP-26's origin-count print titles with explicit row/column start/end bands and add Excel corpus cases for non-origin titles plus first-page/repeat behavior; decide and test a keyboard/a11y printer chooser; either package a redistributable fallback TTF so every PDF embeds a font or document PDF Standard-14 Helvetica as the release contract.
