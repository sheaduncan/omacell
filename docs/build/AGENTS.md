...
`serde`, `serde_json`, `schemars`, `thiserror`, `anyhow` (bins), `rayon`, `smallvec`, `indexmap`, `rustc-hash`, `memchr`, `regex` (with size limits), `chrono` (civil dates only), `unicode-segmentation`, `toml`, `notify`, `tracing`, `tracing-subscriber`, `clap`, `clap_complete`, `clap_mangen`, `zip`, `quick-xml`, `csv`, `encoding_rs`, `calamine` (dev only in host crates; runtime only in the private `.xls` worker), `mlua` (`lua54`, `vendored`), `rmcp`, `tokio` (ai/mcp only), `reqwest` (`rustls-tls`, ai only), `async-trait`, `ratatui`, `crossterm`, `eframe`/`egui`/`egui_kittest`/`wgpu`, `accesskit` (via egui), `fontdb`, `resvg`/`tiny-skia`, `pdf-writer`, `ttf-parser`, `arrow`/`parquet`, `zbus`, `rustix`, `proptest`, `insta`, `assert_cmd`, `criterion`, `cargo-fuzz` (nightly job). Anything else: one justification line in your report and a green `cargo deny check`. License allowlist is in `deny.toml`; GPL-family crates are rejected.
